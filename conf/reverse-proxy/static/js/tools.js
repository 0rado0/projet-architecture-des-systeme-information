...
function setTemplate(idTemplate,idContainer,cList){
    ...
    for(const card of cList){
        ...
        // Use imgUrl if available, otherwise fallback to img_src
        const imageSource = card.imgUrl || card.img_src;

        newContent= clone.firstElementChild.innerHTML
                    .replace(/{{family_src}}/g, card.family_src)
                    .replace(/{{family_name}}/g, card.family_name)
                    .replace(/{{img_src}}/g, imageSource)
                    .replace(/{{name}}/g, card.name)
                    .replace(/{{description}}/g, card.description)
                    .replace(/{{hp}}/g, card.hp)
                    .replace(/{{energy}}/g, card.energy)
                    .replace(/{{attack}}/g, card.attack)
                    .replace(/{{defense}}/g, card.defense || card.defence) // Handle both spelling variations
